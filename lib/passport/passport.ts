 ...
 *
 * `privateKey` and `balance` are generated and consumed in the browser; only
 * the proof and its four public inputs are ever sent to the network.
 *
 * The on-chain read helpers (verifyOnChain / getPassport / authorizePayment /
 * isRegistered) are isomorphic — they run from a Server Component / route
 * handler too, so the x402 settlement gate can call them server-side.
 * `mintPassport` and `replaySpentProof` are browser-only.
 */
import type * as snarkjs from "snarkjs";
import { Buffer } from "buffer";
import { Client, networks, Errors } from "./validator-client";
...
// Artifacts live under Open Stellar's /public/zk, served at the site root.
const BASE = "/";
const ART = {
...
const rndAgentId = () =>
  BigInt(
    "0x" + [...crypto.getRandomValues(new Uint8Array(5))].map((b) => b.toString(16).padStart(2, "0")).join(""),
  ).toString();
 ...
 * Browser-only (snarkjs is loaded lazily so it never lands in a server bundle).
 */
 ...
 * whose proven (hidden) spend cap covers it. Pure on-chain reads, so this
 * runs from the settlement route handler.
 */
 ...
 * The chain rejects it with NullifierUsed — anti-replay, live. Browser-only.
 */
